...
#VARIABLES
    #tickers
magic = {'magic': "wstestLTCcomp_v1.31"}
    #tickers
...
ltcbtc = {'price':0.0001, 'best_ask':0.0, 'best_bid':0.0}
...
def ticker():
    ...
    while True:
        try:
            ...
            system("clear")
            print("        COMBO WSTickerLTC BTC->LTC->USD ver 1.31 - 1/25/18")
            if   'BTC-USD' in result['product_id']:
                ...
                ltcusd = result
            elif 'LTC-BTC' in result['product_id']:
                print('ETH-BTC: ' + str(round(float(result['price']),7)), "  Timestamp:", str(result['time']), '\n')
                ltcbtc = result

            print("   BTC-USD     BCH-USD     ETH-USD     LTC-USD     LTC-BTC  ")
            print(" ", round(float(btcusd['price']),2) , "   ", round(float(bchusd['price']),2) , "     ", round(float(ethusd['price']),2) , "    ", round(float(ltcusd['price']),2) , "    ", round(float(ltcbtc['price']),4), "\n")
...
def arbitrage(inv):
    ...
    while True:
        try:
            ...
            ltc = btc / float(str(ltcbtc['best_ask']))
            usd = ltc * float((ltcusd['best_bid']))
...
def arbitrage_opp(inv):
    ...
    while True:
        try:
            #print("in loop")
            ltc = inv / float(str(ltcusd['best_ask']))
            btc = ltc * float(str(ltcbtc['best_bid']))
...
def transact(amount):
    ...
    try:
        ...
        ltc_lot = round(btc_lot / float(ltcbtc['best_ask']),7)
        # Buy 0.01 LTC @ 100 BTC
        response_ltcbtc = auth_client.buy(price=ltcbtc['best_ask'], #USD
                   size=str(ltc_lot), #BTC
                   product_id='LTC-BTC')
        pprint(response_ltcbtc)
        #     Trans 3 LTC -> USD
        # Sell 0.01 LTC @ 200 USD
        response_ltcusd = auth_client.sell(price=ltcusd['best_bid'], #USD
                   size=str(ltc_lot), #BTC
                   product_id='LTC-USD')
        pprint(response_ltcusd)
        ...
        response_ltcbtc.update(magic)
        magic.update({'trns_role':'exit'})
        response_ltcusd.update(magic)
        collection.insert(response_btcusd)
        collection.insert(response_ltcbtc)
        collection.insert(response_ltcusd)
        trns_runs += 1
    except BaseException as err:
...
def transact_opp(amount):
    ...
    #     Trans 1 USD -> BTC
    try:
        ltc_lot = round(amount / float(ltcusd['best_ask']),8)
        # Buy 0.01 BTC @ 100 USD
        response_ltcusd = auth_client.buy(price=ltcusd['best_ask'], #USD
                   size=str(ltc_lot), #BTC
                   product_id='LTC-USD')
        #     Trans 2 BTC -> LTC
        pprint(response_ltcusd)
        btc_lot = round(ltc_lot * float(ltcbtc['best_bid']),7)
        # Buy 0.01 LTC @ 100 BTC
        response_ltcbtc = auth_client.sell(price=ltcbtc['best_bid'], #USD
                   size=str(ltc_lot), #BTC
                   product_id='LTC-BTC')
        pprint(response_ltcbtc)
        #     Trans 3 LTC -> USD
...
tempqt = public_client.get_product_ticker(product_id='LTC-USD')
ltcusd['price'] = tempqt['price']
ltcusd['best_ask'] = tempqt['ask']
ltcusd['best_bid'] = tempqt['bid']
tempqt = public_client.get_product_ticker(product_id='LTC-BTC')
ltcbtc['price'] = tempqt['price']
ltcbtc['best_ask'] = tempqt['ask']
ltcbtc['best_bid'] = tempqt['bid']
