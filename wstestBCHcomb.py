...
bchbtc = {'price':0.0001, 'best_ask':0.0, 'best_bid':0.0}
...
def ticker():
    ...
    while True:
        try:
            ...
            system("clear")
            print("       COMBO WSTickerBCH BTC->BCH->USD ver 1.31 - 1/25/18")
            if   'BTC-USD' in result['product_id']:
                ...
                ethusd = result
            elif 'BCH-BTC' in result['product_id']:
                print('BCH-BTC: ' + str(round(float(result['price']),2)), "  Timestamp:", str(result['time']), '\n')
                bchbtc = result
                ...
                print('LTC-USD: ' + str(round(float(result['price']),7)), "  Timestamp:", str(result['time']), '\n')
                ltcusd = result

            print("   BTC-USD     BCH-USD     ETH-USD     BCH-BTC     ETH-BTC  ")
            print(" ", round(float(btcusd['price']),2) , "   ", round(float(bchusd['price']),2) , "     ", round(float(ethusd['price']),2) , "    ", round(float(bchbtc['price']),8) , "    ", round(float(ethbtc['price']),4), "\n")
            ...
            print("\nCurrent arbitrage level opp: %", round(gain_opp/inv*100,4),"    TRANSACTIONS DONE:",trns_runs_opp,'\n')
...
def arbitrage(inv):
    ...
    while True:
        try:
            ...
            bch = btc / float(str(bchbtc['best_ask']))
            usd = bch * float((bchusd['best_bid']))
...
def arbitrage_opp(inv):
    ...
    while True:
        try:
            #print("in loop")
            bch = inv / float(str(bchusd['best_ask']))
            btc = bch * float(str(bchbtc['best_bid']))
...
def transact(amount):
    ...
    try:
        ...
        bch_lot = round(btc_lot / float(bchbtc['best_ask']),8)
        # Buy 0.01 LTC @ 100 BTC
        response_bchbtc = auth_client.buy(price=bchbtc['best_ask'], #USD
                   size=str(bch_lot), #BTC
                   product_id='BCH-BTC')
        pprint(response_bchbtc)
        #     Trans 3 LTC -> USD
        # Sell 0.01 LTC @ 200 USD
        response_bchusd = auth_client.sell(price=bchusd['best_bid'], #USD
                   size=str(bch_lot), #BTC
                   product_id='BCH-USD')
        pprint(response_bchusd)
        ...
        response_bchbtc.update(magic)
        magic.update({'trns_role':'exit'})
        response_bchusd.update(magic)
        collection.insert(response_btcusd)
        collection.insert(response_bchbtc)
        collection.insert(response_bchusd)
        trns_runs += 1
    except BaseException as err:
...
def transact_opp(amount):
    ...
    #     Trans 1 USD -> BTC
    try:
        bch_lot = round(amount / float(ethusd['best_ask']),8)
        # Buy 0.01 BTC @ 100 USD
        response_bchusd = auth_client.buy(price=bchusd['best_ask'], #USD
                   size=str(bch_lot), #BTC
                   product_id='BCH-USD')
        #     Trans 2 BTC -> LTC
        pprint(response_bchusd)
        btc_lot = round(bch_lot * float(bchbtc['best_bid']),7)
        # Buy 0.01 LTC @ 100 BTC
        response_bchbtc = auth_client.sell(price=bchbtc['best_bid'], #USD
                   size=str(bch_lot), #BTC
                   product_id='BCH-BTC')
        pprint(response_bchbtc)
        #     Trans 3 LTC -> USD
...
tempqt = public_client.get_product_ticker(product_id='BCH-BTC')
bchbtc['price'] = tempqt['price']
bchbtc['best_bid'] = tempqt['bid']
bchbtc['best_ask'] = tempqt['ask']
tempqt = public_client.get_product_ticker(product_id='BCH-USD')
bchusd['price'] = tempqt['price']
bchusd['best_bid'] = tempqt['bid']
bchusd['best_ask'] = tempqt['ask']
#TICKER THREAD INIT
